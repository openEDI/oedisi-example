...
OPFComponent = component_from_json(
    "opf_federate/component_definition.json", bad_type_checker
)
...
if isfile(args.system_json):
    ...
    with open("test_system_runner_ieee123_gadal_w_opf.json", "w") as f:
        f.write(runner_config.json(indent=2))
        print("helics run --path=test_system_runner_ieee123_gadal_w_opf.json")
else:
