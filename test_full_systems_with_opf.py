...
if isfile(args.system_json):
    ...
    print(f"====wiring diagrams uploaded===")
    ...
    print(f"====runner configs for all simulators generated=== ")
    with open("test_system_runner.json", "w") as f:
        f.write(runner_config.json(indent=2))
        print(f"running co-simulation using command=== \n"
              f"helics run --path=test_system_runner.json")
else:
