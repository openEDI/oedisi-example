...
def snapshot_run(dss):
    ...
    """
    dss.run_command('batchedit regcontrol..* enabled=yes')
    dss.run_command('Batchedit Load..* enabled=yes')
...
def get_pvSystems(dss):
    ...
    while PV_flag:
        ...
        PVkW = dss.run_command('? PVsystem.' + PVname + '.Pmpp')
        PVpf = dss.run_command('? PVsystem.' + PVname + '.pf')
        PVkVA = dss.run_command('? PVsystem.' + PVname + '.kVA')
        PVkV = dss.run_command('? PVsystem.' + PVname + '.kV')
