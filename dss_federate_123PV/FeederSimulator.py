...
class FeederSimulator(object):
    ...
    def __init__(self, config):
        ...
        # self.get_opf_system_mats_vecs()
        #
        # # get incidence matrices
        # self.get_opf_incidence_matrices()
        # Control vars dictionary to be sent by OPF
        # self.opf_vals_dict = {}

        # self.set_opf_control_vars()
    ...
    def set_gen_pq_timeseries(self, loadshape_p):
        ...
        for gen in self._gens:
            name = gen["name"]
            dss.run_command(
                'edit Generator.' + str(name) + ' kW=' + str(float(gen['kW']) * df_for_current_time[name].real)
    ...
    def set_load_pq(self, loadshape_p=1., loadshape_q=1.):
        ...
        for load in self._loads:
            dss.run_command(
                'edit Load.' + str(load["name"]) + ' kW=' + str(float(load['kW']) * loadshape_p) + ' kVar=' + str(
                    float(load['kVar']) * loadshape_q))

    def set_pv_pq(self, loadshape_p=1., loadshape_q=1.):
        for pv in self._pvs:
            dss.run_command(
                'edit PVSystem.' + str(pv["name"]) + ' kW=' + str(float(pv['kW']) * loadshape_p) + ' kvar=' + str(
                    float(pv['kVar']) * loadshape_q))

    def set_gen_pq(self, loadshape_p=1., loadshape_q=1.):
        for gen in self._gens:
            dss.run_command(
                'edit Generator.' + str(gen["name"]) + ' kW=' + str(float(gen['kW']) * loadshape_p) + ' kvar=' + str(
                    float(gen['kVar']) * loadshape_q))
